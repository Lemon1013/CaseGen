...
配置由进程环境读取。若使用 `.env`，启动 Uvicorn 时显式传入 `--env-file .env`；不使用 `.env` 时可以直接依赖代码默认值。完整变量和默认值见 [`backend/.env.example`](backend/.env.example)。常用项：
...
Wiki 2.0 的链路是：上传文档 → 解析质量预览 → 创建持久化摄入 Job → 解析/分块/分析/写入 → Review → 发布 revision → FTS5 + 启发式混合检索。原文块保留页码、字符范围和条款号，生成引用可以回到 Wiki 页面或原文块。

- API、请求体、响应字段和错误语义：[`docs/wiki-api.md`](docs/wiki-api.md)
- 迁移前备份、迁移验证、摄入失败恢复、数据恢复和页面回滚：[`docs/wiki2-migration-runbook.md`](docs/wiki2-migration-runbook.md)

发布前可用 `backend/scripts/verify_wiki2_migration.py` 将完整 `data/` 复制到 Git 忽略的 `data-backups/`，并只在副本上执行迁移、FTS 重建和 SQLite 完整性检查。

应用启动会执行 Wiki 2.0 SQLite 迁移，并在检测到旧/部分迁移库时创建一次 `app.db.wiki2-pre-migration.bak`。FTS 是可重建索引；不可用时检索自动回退到确定性启发式排序。生产环境请先备份完整 `data/`，不要只备份数据库文件。
...
│   └── meta/                # app.db
├── fixtures/                # 演示样例
├── docs/wiki-api.md         # Wiki 2.0 API
├── docs/wiki2-migration-runbook.md # Wiki 2.0 迁移与恢复手册
...
Wiki 2.0 的完整迁移和恢复流程见 [`docs/wiki2-migration-runbook.md`](docs/wiki2-migration-runbook.md)。恢复时应同时保留 `data/meta/app.db`、`data/wiki/` 和 `data/raw/`，否则数据库中的页面路径或原文引用可能无法打开。
...
- Wiki 2.0 API：[`docs/wiki-api.md`](docs/wiki-api.md)
- Wiki 2.0 迁移手册：[`docs/wiki2-migration-runbook.md`](docs/wiki2-migration-runbook.md)
